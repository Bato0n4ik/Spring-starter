...
public class FilterUserRepositoryImpl implements FilterUserRepository{
    ...
    public void updateCompanyAndRole(List<User> users) {
        var batch = users.stream()
                .map(user -> new Object[]{user.getCompany().getId(), user.getRole().name()}).toList();

        jdbcTemplate.batchUpdate(UPDATE_COMPANY_AND_ROLE, batch);
    }
    ...
    public void updateCompanyAndRoleNamed(List<User> users) {
        ...
        namedJdbcTemplate.batchUpdate(UPDATE_COMPANY_AND_ROLE_NAMED, batch);
    }
